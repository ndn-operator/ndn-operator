...
use operator::{
    self,
    network_controller::{State as NetworkState, run_nw, run_pod_sync, run_router},
    telemetry,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let (_, _, _, _, server_result) =
        tokio::join!(nw_ctrl, rt_ctrl, pod_sync, cert_ctrl, server.run());
    server_result?;
    Ok(())
}
