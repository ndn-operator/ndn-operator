use operator::network_controller::{Network, NetworkSpec, NdndSpec, OperatorSpec};
...
fn test_network_spec() -> NetworkSpec {
    NetworkSpec {
        ...
        operator: Some(OperatorSpec { image: "test/operator:latest".into() }),
...
fn test_network() -> Network {
    ...
    nw.metadata = ObjectMeta { namespace: Some("test-ns".into()), uid: Some("dummy-uid".into()), ..ObjectMeta::default() };
    nw
}
...
fn daemonset_builder_basic() {
    let nw = test_network();
    let ds: DaemonSet = create_owned_daemonset(&nw, Some("opimg".into()), Some("sa".into())).expect("build ds");
    ...
    if let Some(init_ct) = init {
        let env_names: Vec<String> = init_ct.env.unwrap_or_default().into_iter().map(|e| e.name).collect();
